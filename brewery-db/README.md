...
| Sass Lint | lint scss (Run `gulp scss-lint`)                                                                                                                                                    |
## Quickstart
...
I abandoned the api and used the API Explorer
see (http://www.brewerydb.com/developers/app-details/appId/fPVNKt)
From Here I took 10 beers and 10 breweries


Rather than give up, I used Angular to mimic the data.
Featured beer is a random beer of the 10 beers available.

Search will filter beers and breweries by name.
They are ordered by name by default.
When searching for a beer, you can sort the beers alphabetically (A-Z, Z-A),
by strength, by type of beer and can use the dropdown list to quickly see
beer types.

Searching breweries will allow sorting again alphabetically (A-Z, Z-A),
the other sort options such as strength and type don't apply to 
breweries and as a result are hidden.
...
Developed using Sass with browser prefixes added through autoprefixer.
